...
function markdownToLexical(markdown) {
  ...
  const finalMarkdown = updatedMarkdown.replace(/{{< current-date >}}/g, '21 July 2025');
  
  // Create a simple lexical structure with the markdown content
  // Ghost will convert markdown to proper Lexical format
  return {
    root: {
      children: [
        {
...
async function updatePage(pageId, slug, content, metadata) {
  ...
  const updateData = {
    pages: [{
      lexical: JSON.stringify(markdownToLexical(content)),
...
async function main() {
  ...
  for (const page of pagesToUpdate) {
    ...
    try {
      await updatePage(page.id, page.slug, page.data.content, page.data);
    } catch (error) {
